...
目标版本：MVP 1.0
## 1. 结论摘要
Calenda 是一个仅驻留在 macOS 菜单栏的原生日历工具。应用使用 SwiftUI 的 MenuBarExtra 和 window 样式呈现固定尺寸的富内容弹窗，以本地公历计算和 Tyme4Swift 农历计算为核心，辅以中国法定节假日、调休、节气、天气和位置能力。
...
## 5. 界面设计
### 5.1 总体形态
使用 MenuBarExtra 的 window 样式。默认内容尺寸为 660 × 430 pt，允许在 600 × 390 至 760 × 520 pt 范围内适应更大系统文字；月历格保持稳定，详情区在空间不足时可滚动。应用设置 LSUIElement 为 true，不在 Dock 和应用切换器中出现。
```text
...
设置按钮打开单实例设置窗口；窗口已经存在时将其置前，不创建重复窗口。设置窗口出现后菜单栏弹窗自然失焦关闭，但天气和节假日任务由 Repository 持有，不随弹窗销毁。
### 5.3 日期格信息层级
...
- 不提供“隐藏菜单栏图标”设置。用户从系统移除图标导致应用退出时，可从 Applications 或 Spotlight 重新启动恢复。
### 5.7 键盘与辅助功能
...
| 项目 | 选择 | 说明 |
|---|---|---|
| 最低系统 | macOS 13 Ventura | MenuBarExtra 的可用基线，覆盖现代 SwiftUI 菜单栏能力 |
| 语言 | Swift，启用严格并发检查 | 网络、缓存和位置采用 async/await 与 actor 隔离 |
| UI | SwiftUI | MenuBarExtra、Window、Grid/LazyVGrid；设置窗口通过 openWindow(id:) 打开 |
| IDE | 当前稳定版 Xcode | 提交共享 Scheme，禁止提交用户级 Xcode 状态 |
...
## 7. 总体架构
采用轻量的 Feature + Repository + Service 分层，不引入大型状态管理框架。UI 状态由 MainActor 上的 AppModel 统一编排，远程访问和文件缓存由 actor 隔离。
```mermaid
flowchart TD
    UI["MenuBarExtra / Settings Window / SwiftUI Views"] --> VM["AppModel @MainActor"]
    UI --> SC["SettingsCoordinator @MainActor"]
    SC --> SS["SettingsStore @MainActor"]
    SC --> LI["LoginItemService"]
    VM --> CE["CalendarEngine"]
    VM --> LR["LunarRepository"]
    VM --> HR["HolidayRepository actor"]
    VM --> WR["WeatherRepository actor"]
    VM --> LS["LocationService @MainActor"]
    SS --> VM
    CE --> FD["Foundation Calendar"]
    LR --> TY["Tyme4Swift Adapter"]
    HR --> HB["Bundled Holiday Snapshot"]
    HR --> HC["Holiday Cache"]
    HR --> HN["Trusted Mirror Chain"]
    WR --> WC["Weather Cache"]
    WR --> OM["Open-Meteo API"]
    LS --> CL["CoreLocation"]
```
### 7.1 分层职责
- View：只负责渲染、焦点、可访问性和发送用户意图，不发网络请求、不读写文件。
- AppModel：维护 displayedMonth、selectedDay、today、weatherState、holidayState 和 permissionState；处理界面意图。
- CalendarEngine：生成稳定的 6 × 7 月历模型，执行日期移动、月份边界和本地化星期计算。
- LunarRepository：把 DateOnly 转为农历、节气、干支等应用模型，隐藏 Tyme4Swift 类型。
- HolidayRepository：读取内置快照、缓存与远端数据，完成校验、合并、回退和原子更新。
- WeatherRepository：封装 Open-Meteo 请求、缓存、刷新节流和错误分类。
- LocationService：封装 CoreLocation 授权、一次性定位、反向地理编码和位置变更。
- SettingsStore：作为设置的唯一事实源，负责类型化读写、默认值、校验、版本迁移和变更发布；View 不直接散布 UserDefaults 键。
- SettingsCoordinator：执行城市切换、清理缓存、恢复默认值和登录项注册等有副作用的设置操作，只有操作成功后才提交最终状态。
- Settings Window：使用带固定 id 的 Window scene 和 openWindow(id:) 提供单实例设置窗口。
### 7.2 关键协议
```swift
protocol LunarCalendarProviding: Sendable {
    func information(for day: CalendarDayID) throws -> LunarInformation
}
...
- Tyme4Swift 本身也包含法定假日能力，但本项目不将其作为法定调休权威源，避免与 holiday-cn 双源冲突；调休统一由 HolidayRepository 提供。
...
2. CalendarEngine 立即生成 42 个公历单元格。
3. LunarRepository 在本地补齐农历与节气。
4. HolidayRepository 返回缓存或内置快照，并后台决定是否更新。
5. WeatherRepository 立即返回缓存；如需刷新则启动单一后台任务。
...
- 弹窗关闭不取消 Repository 持有的有效刷新；城市变更、应用退出或任务被新请求取代时才取消。
### 13.3 状态类型
```swift
...
## 15. 设置页
### 15.1 窗口入口与兼容策略
- App 声明一个带稳定 WindowID.settings 的单实例 Window scene，内容为 SettingsRootView。
- 菜单栏底部“设置”和 Command + , 都调用 openWindow(id: WindowID.settings)，已经打开时置前。
- 设置窗口默认 560 × 420 pt，可调整大小，关闭窗口不退出菜单栏应用。
### 15.2 信息架构
...
- SettingsStore 是 @MainActor 的 Observable/ObservableObject，接受可注入的 UserDefaults suite，便于隔离测试。
...
### 15.4 登录时启动
- MVP 纳入“登录时启动”，默认关闭，使用 macOS 13 的 SMAppService.mainApp。
...
│   │   ├── TymeLunarCalendarAdapter.swift
...
## 18. 测试策略
### 18.1 单元测试
- CalendarEngine：每月恒定 42 格、周一起始、跨年、闰年、2 月、DST、时区变化和非当前月日期。
...
- 验证设置窗口 id 保证单实例，重复打开只置前。
### 18.3 UI 与可访问性测试
- 弹窗首次打开、翻月、选日、返回今天、打开设置和退出。
- 设置路径覆盖首周、城市搜索、当前位置拒绝、温度单位、登录项失败、恢复默认和清除缓存确认。
- VoiceOver label、键盘遍历、浅色/深色、高对比度、简体中文长文本与系统大文字布局。
...
## 22. 分阶段实施
### Phase 0：工程基线
- 创建 macOS SwiftUI 工程、MenuBarExtra window、单实例设置 Window、LSUIElement、App Sandbox、共享 Scheme。
- 建立 Domain、Services、Features 目录和测试 Target。
- 接入 Tyme4Swift，锁定版本并完成最小适配测试。
- 建立 SettingsStore、schemaVersion、设置迁移和 SMAppService 登录项最小验证。
### Phase 1：离线核心
- 完成 6 × 7 公历、选日、翻月、返回今天、键盘导航。
- 接入农历、节气、今天高亮和右侧详情。
- 完成深浅色、VoiceOver 和基础设置。
...
### Phase 4：发布质量
- 性能分析、UI 冒烟、许可清单、Developer ID 签名、公证和 GitHub Release。
- 完成 README、隐私说明、故障排查和发布检查表。
## 23. MVP 验收标准
- 应用仅显示在菜单栏，点击后稳定打开窗口样式弹窗。
...
| 菜单栏弹窗生命周期短 | 异步结果丢失或重复请求 | Repository 持有任务、请求合并、View 不拥有网络生命周期 |
...
2. 最低系统为 macOS 13，不为 macOS 12 增加 NSStatusItem + NSPopover 兼容层。
3. MVP 提供完整设置入口与设置管理；首周默认周一，用户可切换为周日或跟随系统。
4. MVP 提供默认北京、手动城市和当前位置；日历时区始终跟随 Mac 系统，天气使用城市时区。
5. MVP 纳入登录时启动，默认关闭并通过 SMAppService.mainApp 管理。
6. 正式 GitHub Release 使用 MIT 许可证，并提供签名、公证、SHA-256 完整的 DMG。
## 26. 参考资料
- [Apple MenuBarExtra 文档](https://developer.apple.com/documentation/swiftui/menubarextra)
...
- [Apple SwiftUI Settings 文档](https://developer.apple.com/documentation/swiftui/settings)
